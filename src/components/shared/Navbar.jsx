            ...
            {navItems.map((item) => (
              <li key={item.href}>
                <HeroLink
                  as={NextLink}
                  href={item.href}
                  className="group relative font-medium text-slate-600 transition-all duration-300 hover:text-teal-700"
                >
                  {item.name}

                  <span className="absolute -bottom-2 left-0 h-0.5 w-0 rounded-full bg-teal-600 transition-all duration-300 group-hover:w-full" />
                </HeroLink>
              </li>
            ))}
          </ul>
              ...
              {navItems.map((item) => (
                <li key={item.href}>
                  <HeroLink
                    as={NextLink}
                    href={item.href}
                    className="block rounded-2xl px-4 py-3 text-base font-medium text-slate-700 transition-all duration-300 hover:bg-teal-50 hover:text-teal-700"
                  >
                    {item.name}
                  </HeroLink>
                </li>
